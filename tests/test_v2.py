...
def test_collection_validation_serialization():
    ...
    assert users.model_dump() == [user0.model_dump(), user1.model_dump()]

    users2 = UserCollection.model_validate_json(users.model_dump_json())
    for (u1, u2) in zip(users, users2):
        assert u1 == u2

    users3 = UserCollection.model_validate(users.model_dump())
